...
clf_gs = GridSearchCV(clf_tree, param_grid=params, cv=3)

clf_gs.fit(df_train, y)

#except:
#    print('error on second fit')

print('-- CV Results --\n')
try:
    print(clf_gs.cv_results_)
except:
    print('error on cv_results')
print('---------------------------------------\n')
...
try:
    ...
    print('error on getting best params')
print('---------------------------------------')

y_pred = clf_gs.predict(df_test)
y_pred_train = clf_gs.predict(df_train)

RMSE_test = sqrt(mean_squared_error(y_true, y_pred))
RMSE_train = sqrt(mean_squared_error(y, y_pred_train))

for idx, each in enumerate(clf_gs.feature_importances_):
    print(idx, each*1e5)
